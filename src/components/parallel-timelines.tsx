...
const eventsData: TimelineEvent[] = allEvents.map((event) => ({
  id: event.slug || event._id || crypto.randomUUID(), // 用 slug 或 fallback
  title: event.title,
  description: event.description ?? "",
  impact: Number(event.impact), // 保证为 number
  type: event.type as "point" | "range",
...
function parseDate(dateStr: string): Date {
  if (/^\d{4}-\d{2}-\d{2}$/.test(dateStr)) {
    ...
    console.warn(
      `日期格式 "${dateStr}" 应为 YYYY-MM-DD，已自动视为该月第一天。`
    );
...
function generateTimeMarkers(
  ...
  const lastMarkerMonth = new Date(
    Date.UTC(maxDate.getUTCFullYear(), maxDate.getUTCMonth() + 1, 1)
  );

  while (currentDate < lastMarkerMonth) {
    markers.push({
      date: new Date(currentDate), // Create a copy
      label: `${currentDate.getUTCFullYear()}年${
...
export function ParallelTimelines() {
      ...
      if (containerRef.current) {
        ...
        if (eventsData.length > 0) {
          try {
            ...
            if (
              !isNaN(dateRange.min.getTime()) &&
              !isNaN(dateRange.max.getTime())
            ) {
              const monthsDiff =
                (dateRange.max.getUTCFullYear() -
                  dateRange.min.getUTCFullYear()) *
                  12 +
                dateRange.max.getUTCMonth() -
                dateRange.min.getUTCMonth();
              const validMonthsDiff = Math.max(1, monthsDiff);
              const monthHeight = 100;
              height =
                Math.max(600, validMonthsDiff * monthHeight) +
                paddingTop +
                paddingBottom;
            }
          } catch (e) {
  ...
  const renderSvgTimeline = () => {
    ...
    if (width <= 0 || height <= 0 || isNaN(height))
      return (
    ...
    const renderEvents = (category: "world" | "local" | "personal") => {
      ...
      return sortedCategoryEvents.map((event) => {
        ...
        try {
          ...
          if (isNaN(startY) || (endY !== undefined && isNaN(endY)))
            throw new Error("Invalid date position");
        } catch (e) {
          console.error(e);
          return null;
        ...
        if (event.type === "range" && endY !== undefined) {
          labelBaseY =
            ...
            labelHeight / 2; // Default: center in range
        }

        const currentLabelTopY = labelBaseY;
        if (
          lastLabelBottomY !== null &&
          currentLabelTopY < lastLabelBottomY + labelVerticalPadding
        ) {
        ...
        const tooltipContent = (
          <div className="p-1 space-y-1 max-w-xs text-xs">
            <h4 className="font-semibold text-sm" style={{ color: color.dark }}>
              {event.title}
            </h4>
          ...
          return (
            <g key={`${category}-${event.id}`} className="event-point group">
              ...
              <circle
                cx={x}
                cy={startY}
                r={4 + event.impact * 0.5}
                fill={color.main}
                className="cursor-pointer transition-all group-hover:r-[8] group-hover:opacity-80"
                ...
                <TooltipProvider delayDuration={100}>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <div
                        className="px-2 py-0.5 rounded text-xs font-medium cursor-pointer space-y-0.5 h-full flex flex-col justify-center"
                        style={{
                          backgroundColor: color.bg,
                          color: color.dark,
                          textAlign: category === "personal" ? "right" : "left",
                        }}
                        onClick={() => setSelectedEvent(event)}
                      >
                    ...
                    <TooltipContent
                      side={category === "personal" ? "left" : "right"}
                    >
                      {tooltipContent}
                    </TooltipContent>
                  </Tooltip>
          ...
          return (
            <g key={`${category}-${event.id}`} className="event-range group">
              <rect
                ...
                rx={4}
                ry={4}
                className="cursor-pointer transition-all group-hover:fill-opacity-90"
                onClick={() => setSelectedEvent(event)}
              />
              <line
                x1={laneCenters[category]}
    ...
    return (
      <svg
        width={width}
        height={height}
        className="border rounded-lg bg-gradient-to-b from-white to-gray-50 min-w-[600px]"
      >
        {timeMarkerElements}
        {timelineLines}
        {renderEvents("world")}
    ...
    return (
      <Tabs defaultValue="all" className="w-full">
        <TabsList className="grid grid-cols-4 gap-1 mb-4 bg-gray-100 p-1 rounded-lg sticky top-0 z-10">
          <TabsTrigger
            value="all"
            className="text-xs px-2 py-1 data-[state=active]:bg-white data-[state=active]:shadow rounded"
          >
            全部
          </TabsTrigger>
          <TabsTrigger
        ...
        <TabsContent value="personal">
          <MobileTimeline
            events={eventsByCategory.personal}
            dateRange={dateRange}
            onSelectEvent={setSelectedEvent}
          />
        </TabsContent>
  ...
  const renderEventDetails = () => {
    ...
    return (
      ...
      >
        <div
          className="bg-white rounded-xl shadow-2xl max-w-lg w-full overflow-hidden animate-in fade-in-50 zoom-in-90 duration-300"
          ...
          <div className="p-5 space-y-4">
            <div className="text-sm text-gray-600">
              {selectedEvent.type === "point" ? (
                <span>日期：{formatDate(selectedEvent.startDate)}</span>
              ) : (
                <span>
                  时间段：{formatDate(selectedEvent.startDate)} -{" "}
            ...
            <p className="text-gray-800 text-sm leading-relaxed bg-gray-50 p-3 rounded-md border border-gray-200">
    ...
    <div className="p-2 sm:p-4 space-y-4 bg-gray-100 min-h-screen">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 p-3 bg-white rounded-lg shadow-sm border border-gray-200">
          ...
          <div className="flex items-center gap-1" title="Impact Level 1 to 5">
            {Array.from({ length: 5 }).map((_, i) => (
              <div
                ...
                style={{ backgroundColor: `hsl(210, 10%, ${70 - i * 10}%)` }}
              />
            ))}
        ...
        <div className="flex gap-2 self-end sm:self-center">
          <Button
            variant={view === "desktop" ? "secondary" : "outline"}
            size="sm"
          ...
          <Button
            variant={view === "mobile" ? "secondary" : "outline"}
            size="sm"
      ...
      <div
        ref={containerRef}
        className="relative overflow-auto bg-white rounded-lg shadow border border-gray-200"
      ...
      >
        {eventsData.length === 0 ? (
          ...
          view === "desktop" ? (
            ...
            <div className="p-2">{renderMobileView()}</div>
          ) // Add padding around mobile view
        ) : (
...
function MobileTimeline({
  ...
  const calculateHeight = () => {
    if (isNaN(dateRange.min.getTime()) || isNaN(dateRange.max.getTime()))
      return 500;
    const monthsDiff =
      (dateRange.max.getUTCFullYear() - dateRange.min.getUTCFullYear()) * 12 +
      dateRange.max.getUTCMonth() -
      dateRange.min.getUTCMonth();
    const validMonthsDiff = Math.max(1, monthsDiff);
    const monthHeight = 80;
    return (
      Math.max(400, validMonthsDiff * monthHeight) +
      paddingTopMobile +
      paddingBottomMobile
    );
  };
    ...
    <div className="relative w-full flex justify-center py-4 overflow-hidden">
      <svg width={width} height={height} className="bg-white rounded-lg">
        ...
        {mobileTimeMarkers.map((marker) => {
          ...
          if (
            y <= paddingTopMobile + 5 ||
            y >= height - paddingBottomMobile - 5
          )
            return null; // Extra check for bounds
          return (
        ...
        {sortedEvents.map((event, index) => {
          ...
          try {
            // Error handling for date position
            startY = dateToPosition(
              parseDate(event.startDate),
              dateRange.min,
              dateRange.max,
            ...
            if (isNaN(startY) || (endY !== undefined && isNaN(endY)))
              return null; // Skip if position invalid
          } catch (e) {
            console.error(e);
            return null;
          }
          ...
          if (direction === -1) {
            // Left side
            if (
              lastLabelBottomY_Left !== null &&
              currentLabelTopY <
                lastLabelBottomY_Left + labelVerticalPaddingMobile
            ) {
            ...
            // Right side
            if (
              lastLabelBottomY_Right !== null &&
              currentLabelTopY <
                lastLabelBottomY_Right + labelVerticalPaddingMobile
            ) {
                ...
                >
                  <TooltipProvider delayDuration={100}>
                    ...
                    <Tooltip>
                      ...
                      <TooltipTrigger asChild>
                        <div
                          className="px-1.5 py-0.5 rounded text-[10px] font-medium cursor-pointer space-y-0.5 h-full flex flex-col justify-center"
                          style={{
                            backgroundColor: color.bg,
                            color: color.dark,
                      ...
                      <TooltipContent side={direction < 0 ? "left" : "right"}>
                        {tooltipContent}
                      </TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
            ...
            return (
              <g key={`mobile-${event.id}`} className="event-range group">
                <rect
                  ...
                  rx={3}
                  ry={3}
                  className="cursor-pointer transition-all group-hover:fill-opacity-80"
                  onClick={() => onSelectEvent(event)}
                />
                <line
                  x1={centerX}
